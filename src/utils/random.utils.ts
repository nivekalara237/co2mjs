...
export class RandomUtils {
    ...
    private static ALPHA_NUMERIC_CHARS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
     ...
     */
    public static rangeBytes = (length: number, range: { min: number, max: number }): ArrayBuffer => {
        if (length <= 0 || length > Number.MAX_SAFE_INTEGER) {
            ThrowableUtils.raise("byteLength must between 1 and 2^53 − 1", "byteLength");
        }
