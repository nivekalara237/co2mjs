...
import {random, randomInteger} from "../../src/utils/cryptographically";
...
describe("Random Utils Tests", ()=> {
    ...
    it('should generate an int number randomly', () => {
        expect(Number.isInteger(RandomUtils.nextInt()))
            .toBeTruthy();
    });

    it('should generate an octal decimal value', () => {
        expect(Number.isInteger(RandomUtils.nextOctal()))
            .toBeTruthy();
    ...
    it('should generate a hexadecimal value', () => {
        ...
        expect("0123456789ABCDEF".split(""))
            .toContain(RandomUtils.nextHex());
    });
    ...
    it('should throw error when generating a random string', () => {
        ...
        expect(()=> RandomUtils.chars(-2)).toThrow("The length must higher than 0");
    });
    ...
    it('should throw error when generating bytes', () => {
        expect(()=> RandomUtils.rangeBytes(-6, {min: 2, max: 9}))
            .toThrow("byteLength must between 1 and 2^53 − 1");
        expect(()=> RandomUtils.rangeBytes(Number.MAX_VALUE + 8, {min: 2, max: 9}))
            .toThrow("byteLength must between 1 and 2^53 − 1");
        expect(()=> RandomUtils.rangeBytes(12, {min: 2, max: 2}))
            .toThrow("The min and max must be different value");
        expect(()=> RandomUtils.rangeBytes(12, {min: 4, max: 1}))
            .toThrow("The max must be greater than min");
    });
