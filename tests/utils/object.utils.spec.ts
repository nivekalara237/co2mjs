...
describe("ObjectUtils", ()=> {
    ...
    describe("appendDefined", () => {
        ...
        it('should append child to undefined objet', () => {
            expect(ObjectUtils.appendDefined(undefined, "name", "apples")).toBeUndefined()
        ...
        it('should append boolean child to objet', () => {
            const obj = {weight: "apples"};
            expect(ObjectUtils.appendDefined(obj, "circle", true)).toEqual({...obj, circle: true})
        });

        it('should append bigint child to objet', () => {
            const obj = {weight: "apples"};
            expect(ObjectUtils.appendDefined(obj, "area", 70980980980980980898008n)).toEqual({...obj, area: 7.098098098098098e+22})
        });

        it('should append number child to objet', () => {
            const obj = {weight: "apples"};
            expect(ObjectUtils.appendDefined(obj, "area", 9)).toEqual({...obj, area: 9})
        });
        ...
        it('should append string child to objet', () => {
            const obj = {weight: "apples"};
            expect(ObjectUtils.appendDefined(obj, "desc", "fruit")).toEqual({...obj, desc: "fruit"});
        });

        it('should append object child to objet', () => {
            const obj = {weight: "apples"};
            expect(ObjectUtils.appendDefined(obj, "category", {name: "fruit"})).toEqual({...obj, category: {name: "fruit"}})
        });
    })
    ...
    it('should compute the size of boolean,bigint,number,etc', () => {
        ...
        expect(ObjectUtils.size(2.90)).toEqual(0);
        expect(ObjectUtils.size(()=>{})).toEqual(0);
    });
})
